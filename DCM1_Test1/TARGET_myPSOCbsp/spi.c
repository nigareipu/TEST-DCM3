...
void DiscrDAC_SPI_Init(void)
{
	...
	cyhal_gpio_init(P6_2, CYHAL_GPIO_DIR_OUTPUT, CYHAL_GPIO_DRIVE_PULLUP, 1);
	/*Configure the SPI Master settings*/
	cyhal_spi_init(&DiscrDAC_obj, P2_0, NC, P2_2, P2_3, NULL, 8, CYHAL_SPI_MODE_00_MSB, false);
	/*Set the SPI frequency*/
	cyhal_spi_set_frequency(&DiscrDAC_obj, spi_master_frequency);
}
...
void HVDAC_SPI_Init(void)
{
	...
	uint32_t spi_master_frequency = 1000000;
	// Initialize CLR pin to high so DAC registers can be updated
	cyhal_gpio_init(P6_3, CYHAL_GPIO_DIR_OUTPUT, CYHAL_GPIO_DRIVE_PULLUP, 1);
	/*Configure the SPI Master settings*/
	cyhal_spi_init(&HVDAC_obj, P11_0, NC, P11_2, P11_3, NULL, 8, CYHAL_SPI_MODE_00_MSB, false);
	/*Set the SPI frequency*/
	cyhal_spi_set_frequency(&HVDAC_obj, spi_master_frequency);
}
